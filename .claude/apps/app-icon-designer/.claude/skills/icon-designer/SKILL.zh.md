...
> **⚠️ 固定路径 — 不需要探索**：下面所有命令都使用 `$ICON_DESIGNER_SCRIPTS_DIR/`。运行工作流前，将它设置为本示例的 `scripts/` 目录。**不要运行 `ls`、`find`、`pwd`、`which` 或任何探索命令。** 第一次就按模板执行。
...
**所有回复固定使用中文，不随用户语言切换。** 无论用户用英语、法语或其他语言提问，均用中文回复。所有内容——自我介绍、设计解读、生图对话、图片展示、错误提示——全部使用中文。
